...
{
    public partial class FrmMainPK_Socket_2 : Form
    {
        ...
        private void FrmMainSocket_2_Load(object sender, EventArgs e)
        {
            try
            {
                try
                ...
                serviceObjs = serviceObjs.Where(x => serviceIds.Contains(x.Id)).OrderBy(x => x.Data).ToList();
                ...
                if (loginObj != null)
                {
                    ...
                    lbUserName.Text = "Nhân viên: " + loginObj.UserName + " | Giờ đăng nhập: " + loginObj.LoginTime + " | IP Address: " + BaseCore.Instance.GetLocalIPAddress();
                }
                ...
                if (serviceObjs.Count > 0)
                {
                    ...
                    for (int i = 0; i < serviceObjs.Count; i += soNutDV1dong)
                    {
                        for (int ii = 0; ii < soNutDV1dong; ii++)
                        {
                            try
                            {
                                obj = serviceObjs[current]; //serviceObjs.FirstOrDefault(a => a.Id == serviceIds[current]); 
                                if (obj != null)
                                {
                ...
                switch (printType)
                {
                    case (int)ePrintType.TheoTungDichVu:
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                var rs = BLLHuuNghi.Instance.PrintNewTicket(connectString, e.Require.ServiceId, serObj.StartNumber, 0, now, printType, e.Require.ServeTime.TimeOfDay, txtname.Text, txtAdd.Text, ((int)txtDOB.Value), txtma.Text, "", "", "", "", "", (int)eDailyRequireType.KhamBenh, "");
                                if (rs.IsSuccess)
                                {
                    ...
                    case (int)ePrintType.BatDauChung:
                        ...
                        else
                        {
                            ...
                            {
                                //  var rs = BLLDailyRequire.Instance.PrintNewTicket(connectString,, startNumber, 0, now, printType, (e.Require.ServeTime != null ? e.Require.ServeTime.TimeOfDay : serObj.TimeProcess.TimeOfDay), txtname.Text, txtAdd.Text, (int)txtDOB.Value, txtma.Text, "", "", "", "", "");
                                var rs = BLLHuuNghi.Instance.PrintNewTicket(connectString, e.Require.ServiceId, startNumber, 0, now, printType, (e.Require.ServeTime != null ? e.Require.ServeTime.TimeOfDay : serObj.TimeProcess.TimeOfDay), txtname.Text, txtAdd.Text, (int)txtDOB.Value, txtma.Text, "", "", "", "", "", (int)eDailyRequireType.KhamBenh, "");
                                if (rs.IsSuccess)
                                {
                                    lastTicket = (int)rs.Data;
                                    nghiepVu = rs.Data_1;
        ...
        {
            if (printSerialCOM != null && printSerialCOM.IsOpen)
            {
                ...
                for (int ii = 0; ii < so_lien; ii++)
                    for (int i = 0; i < arr.Length; i++)
                    {
                        try
                        {
                            BaseCore.Instance.PrintTicketTCVN3(printSerialCOM, arr[i]);
                        }
                        catch (Exception ex)
                        {
